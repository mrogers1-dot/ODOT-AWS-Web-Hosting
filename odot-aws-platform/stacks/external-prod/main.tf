# main.tf — External Prod stack composition.
#
# Wires together the five platform modules (networking, ecs-cluster, security,
# monitoring, oidc) for the external-prod account-stage combination.
...
# ── Networking ────────────────────────────────────────────────────────────────
#
# Provisions the external-prod VPC with public subnets (IGW-attached) and
...
# ── ECS Cluster ───────────────────────────────────────────────────────────────
#
# Provisions the ECS Fargate cluster with FARGATE (on-demand) as the default
# capacity provider (prod stage) and Container Insights enabled.
module "ecs_cluster" {
